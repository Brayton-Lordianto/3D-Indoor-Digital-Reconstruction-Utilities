...
def megasam_to_nerfstudio(npz_path, output_dir, image_dir="images"):
    """
    Convert MegaSAM npz file to Nerfstudio format.
...
def convert_megasam_to_nerfstudio(npz_path, output_dir, image_dir="images"):
    ...
    pixels = np.stack([xx, yy, np.ones_like(xx)], axis=-1).reshape(-1, 3).T
    ...
    for i in range(len(images)):
        ...
        rgb = images[i].reshape(-1, 3) / 255.0
        pose = poses[i]
        
        pts_cam = Kinv @ pixels * depth
        pts_cam = np.vstack([pts_cam, np.ones((1, pts_cam.shape[1]))])
